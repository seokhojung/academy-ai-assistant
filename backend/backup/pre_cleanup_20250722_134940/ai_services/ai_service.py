...
from app.services.openai_service import OpenAIService as NewOpenAIService
...
class OpenAIService(AIService):
    """OpenAI 서비스 래퍼 - 새로운 OpenAI 전용 파일 사용"""
    
    def __init__(self):
        self._service = NewOpenAIService()
    
    async def generate_response(self, message: str, session: Optional[Session] = None) -> str:
        """OpenAI API를 사용한 응답 생성"""
        return self._service.generate_response(message)
    
    async def generate_stream(self, message: str) -> AsyncGenerator[str, None]:
        """OpenAI API를 사용한 스트리밍 응답 생성"""
        response = self._service.generate_response(message)
